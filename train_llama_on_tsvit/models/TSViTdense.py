...
class TSViT(nn.Module):
    ...
    def forward(self, x):
        ...
        x += self.space_pos_embedding  # [:, :, :(n + 1)]
        x = self.dropout(x)
        embeddings = self.space_transformer(x)  # Store embeddings before MLP head
        
        # Get mask predictions
        x = self.mlp_head(embeddings.reshape(-1, self.dim))
        x = x.reshape(
            B, self.num_classes, self.num_patches_1d**2, self.patch_size**2
        ).permute(0, 2, 3, 1)
        x = x.reshape(B, H, W, self.num_classes)
        mask = x.permute(0, 3, 1, 2)
        
        return mask, embeddings  # Return both mask and embeddings
