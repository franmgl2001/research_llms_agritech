...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        if len(sample["labels"].shape) == 3:  # Check if the shape is three-dimensional
            ...
            tensor_sample["labels"] = (
                torch.tensor(sample["labels"].astype(np.float32))
                .to(torch.float32)
                .unsqueeze(-1)
            )
...
class CutOrPad(object):
    ...
    def pad_or_cut(self, tensor, dtype=torch.float32):
        ...
        if diff > 0:
            ...
            random_indices = torch.randperm(len(tensor))[self.max_seq_len]
            #tensor = tensor[random_indices]
